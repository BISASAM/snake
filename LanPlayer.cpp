...
sf::Texture LanPlayer::arrowtxt;


LanPlayer::LanPlayer()
{}

void LanPlayer::create(sf::Color col, sf::Vector2<int> pos, sf::Keyboard::Key K1, sf::Keyboard::Key K2)
{
	circle.setRadius(4);
...
float LanPlayer::norm(sf::Vector2f r)
{
	int l= sqrtf(r.x*r.x + r.y*r.y);
	return l;
}

void LanPlayer::Update(float &elapsedTime)
{
	eT = elapsedTime;
...
void LanPlayer::PchangeSpeed(float s)
{
	speed = speed*s;
}

void LanPlayer::PchangeThickness(int d)
{
...
void LanPlayer::Timerdesolve() //Liste der PowerUp-Timer abarbeiten und PowerUps rueckgaengig machen
{
...
//server
sf::Vector2f LanPlayer::getHead()
{
	return sf::Vector2f(circle.getPosition());
}

void LanPlayer::Collision()
